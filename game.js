...
function scoreState() {
  ...
  if (keyState[32]) {
    if (score == 3 || score == -3) gameState = endGameState;
    else {
      resetPlayField();
      gameState = playState;
    }
...
function playState() {
  if (keyState[A]) slime1.pos.x = Math.max(50, slime1.pos.x - sidestep);
  if (keyState[D])
    slime1.pos.x = Math.min(
      WIDTH / 2 - slimeRadius - wallWidth / 2,
      slime1.pos.x + sidestep
    );
  if (slime1.pos.y == 0 && keyState[W]) slime1.yspeed = jumpSpeed;

  slime1.pos.y = Math.max(0, slime1.pos.y + slime1.yspeed);
  if (slime1.pos.y > 0) slime1.yspeed -= gravity;

  if (keyState[LEFT])
    slime2.pos.x = Math.max(
      WIDTH / 2 + slimeRadius + wallWidth / 2,
      slime2.pos.x - sidestep
    );
  if (keyState[RIGHT])
    slime2.pos.x = Math.min(WIDTH - slimeRadius, slime2.pos.x + sidestep);
  if (slime2.pos.y == 0 && keyState[UP]) slime2.yspeed = jumpSpeed;

  slime2.pos.y = Math.max(0, slime2.pos.y + slime2.yspeed);
  if (slime2.pos.y > 0) slime2.yspeed -= 0.5;
...
function endRoundState() {
  ...
  ctx.font = "20pt Arial";
  if (score > 0) ctx.fillText("Yellow slime scores!", 120, 100);
  else ctx.fillText("Blueish slime scores!", 120, 100);
...
function endGameState() {
  ...
  if (score > 0) ctx.fillText("Yellow slime is the winner!", 100, 150);
  else ctx.fillText("Blueish slime is the winner!", 100, 150);
}

function checkCollisions() {
  var distSquared1 =
    Math.pow(slime1.pos.x - ball.pos.x, 2) +
    Math.pow(slime1.pos.y - ball.pos.y, 2);
  var distSquared2 =
    Math.pow(slime2.pos.x - ball.pos.x, 2) +
    Math.pow(slime2.pos.y - ball.pos.y, 2);
  ...
  if (
    ball.pos.x + ballRadius > WIDTH / 2 - wallWidth &&
